...
public class PlaneVisualizationManager : MonoBehaviour {
    ...
    private List<GameObject> _spawnedPlanes;

    private void Start()
    {
        _spawnedPlanes = new List<GameObject>();
    }
	...
	{     
	    ...
	    foreach (var curPlane in _newPlanes)
	    {
	        ...
	        var planeObject = Instantiate(TrackedPlanePrefab, Vector3.zero, Quaternion.identity,
	            transform);
            _spawnedPlanes.Add(planeObject);
    ...
    // destroy all spawned planes
    public void DestroySpawnedPlanes()
    {
        foreach (var tPlanes in _spawnedPlanes)
        {
            Destroy(tPlanes);
        }
    }
